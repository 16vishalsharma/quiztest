...
$response = ""; 
	...
	exit();
		...
		$countCheck = mysqli_query($conn,"SELECT id FROM pquestions")or die(mysqli_error());
	...
	if(!in_array("1", $_SESSION['answer_array'])){
		$sql = mysqli_query($conn,"INSERT INTO presult (username, percentage, date_time) 
		VALUES ('$username', '0', now())")or die(mysqli_error($conn));
		echo "Did you even read the questions? You scored $percent%, You must be retarded.";
		echo '<br/><br/><br/><a href="student.php" style="width:200px; height:40px;background-color:green;color:white;border:2px solid black;">Go to student login page</a>';
	...
	$sql = mysqli_query($conn,"INSERT INTO presult (username, percentage, date_time) 
	VALUES ('$username', '$percent', now())")or die(mysqli_error($conn));
