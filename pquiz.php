...
if(isset($_GET['question'])){
	...
	if(!isset($_SESSION['qid_array']) && $question != 1){
		...
		header("location: pindex.php?msg=$msg");
		exit();
	}
...
xhr.send(vars);
		...
		document.getElementById('btnSpan').innerHTML += '<a href="pquiz.php?question=<?php echo $next; ?>">Click here now</a>';
...
hr.open("GET", "pquestion.php?question=" + <?php echo $question; ?>, true);
  hr.send();
}
function x() {
...
function post_answer(){
	var p = new XMLHttpRequest();
		...
		if(p.readyState == 4 && p.status == 200) {
			...
			var url = 'pquiz.php?question=<?php echo $next; ?>';
			window.location = url;
	}
}
p.send(vars);
